 ...
 * @throws {CustomError}
 */
const uploadImageToService = async ({ imageDataBuffer, originalFileName, mimeType }) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new CustomError({
                message: errorData.message || `Image upload failed with status: ${response.status}`,
...
const updateArticle = async (req) => {
    ...
    const updatedArticle = await articleRepository.update(id, {
        ...
        is_published: is_published !== undefined ? (is_published === 'true') : existingArticle.is_published,
    });
